#!/bin/bash
# Fixed deployment script for Silent Connect on DigitalOcean
# Usage: ./deploy-fixed.sh

set -e  # Exit on any error

echo "🚀 Deploying Silent Connect to DigitalOcean (Fixed)..."
...
# Install required packages (avoid upgrade to prevent conflicts)
echo "📦 Installing required packages..."
apt install -y nginx php8.1-fpm php8.1-sqlite3 php8.1-mbstring php8.1-xml php8.1-curl php8.1-gd php8.1-zip php8.1-mysql unzip git
...
sed -i 's/upload_max_filesize = .*/upload_max_filesize = 50M/' /etc/php/8.1/fmp/php.ini
...
sed -i 's/memory_limit = .*/memory_limit = 256M/' /etc/php/8.1/fmp/php.ini
